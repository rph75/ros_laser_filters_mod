...
class Driver:
    ...
    def __init__(self, addr):
        ...
        self._reg_write_bits(registers.IF_CONF, definitions.IF_MODE_MAG,definitions.IF_MODE_BIT,definitions.IF_MODE_LEN)
        ...
        sleep_ms(1)
        # Wait for power-up to complete
        while (1 != self._reg_read_bits(registers.PMU_STATUS, definitions.MAG_PMU_STATUS_BIT,
                                        definitions.MAG_PMU_STATUS_LEN)):
            sleep_ms(200)
            pass
        sleep_ms(1)
    ...
    # Get magnetomter output data rate.
    # The mag_odr parameter allows setting the output data rate of the magnetomter
    # as described in the table below.
    #
    # <pre>
    #  6 =   25Hz
    ...
    #
    # @return Current sample rate
    # @see registers.MAG_CONF
    # @see BMI160MagRate
    def get_mag_rate(self):
        return self._reg_read_bits(registers.MAG_CONF, definitions.MAG_RATE_SEL_BIT, definitions.MAG_RATE_SEL_LEN)

    # Set magnetometer output data rate.
    # Note this is only the rate at which the data is copied from the magnetometer
    # The actual capture rate on the magnetometer must be set independently using set_mag_catpure_rate()
    # @param rate New output data rate
    # @see get_mag_rate()
    # @see definitions.MAG_RATE_25HZ
    # @see registers.MAG_CONF
    def set_mag_rate(self, rate):
        self._reg_write_bits(registers.MAG_CONF, rate,
                             definitions.MAG_RATE_SEL_BIT, definitions.MAG_RATE_SEL_LEN)

    #
    # Set the I2C address of the magnetormeter
    def setMagI2Caddress(self, mag_address):
        mag_address = mag_address & 0xFE #LSB must be 0
        self._reg_write(registers.MAG_IF_0, mag_address)

    #
    # Performs a soft reset of the magnetometer and brings it into normal (not suspended) state
    def mag_soft_reset(self):
        self.writeToMagRegister(registers.BMM150_POWER, 0x83) #Set both soft reset flags, and the power control bit
        sleep_ms(1)
    ...
    # @see registers.FIFO_CONFIG_1
    def getMagFIFOEnabled(self):
        return 0 != (self._reg_read_bits(registers.FIFO_CONFIG_1, definitions.FIFO_MAG_EN_BIT, 1))

    # Set magnetometer FIFO enabled value.
    # @param enabled New magnetometer FIFO enabled value
    # @see getMagFIFOEnabled()
    # @see registers.FIFO_CONFIG_1
    def setMagFIFOEnabled(self, enabled):
        self._reg_write_bits(
            registers.FIFO_CONFIG_1, 0x1 if enabled else 0, definitions.FIFO_MAG_EN_BIT, 1)

    # False: Set magnetometer to manual mode (setup mode)
    # True: Set magnetometer to data mode
    def setMagManual(self, enabled):
        self._reg_write_bits(
            registers.MAG_IF_1, 0x1 if enabled else 0, definitions.MAG_IF_1_MANUAL_ENABLED_BIT, 1)

    # Set magnetometer capture rate.
    # Note this is only the rate at which the data is copied from the magnetometer
    # The actual capture rate on the magnetometer must be set independently using set_bmm_rate()
    # @param rate New output data rate
    # @see get_mag_rate()
    # @see definitions.BMM_RATE_10HZ
    # @see registers.BMM_CONF
    def set_mag_catpure_rate(self, rate):
        #First, set the mag rate on the magnetometer itself
        ctrl = (rate << definitions.BMM150_ODR_BIT)
        # This will set opmode to 0 (normal)
        self.writeToMagRegister(registers.BMM150_CONTROL,ctrl)

    # Writes to a magnetometer register
    def writeToMagRegister(self,register,value):
        self.setMagManual(True)
        #Write the data to MAG_IF[4]
        self._reg_write(registers.MAG_IF_4,value)
        #Write the register address to MAG_IF[3], which will trigger the actual write operation
        self._reg_write(registers.MAG_IF_3, register)
        #Wait for write to complete by observing bit in status register
        while (self._reg_read_bits(registers.STATUS, definitions.STATUS_MAG_MAN_OP, 1)):
            sleep_ms(1)
        self.setMagManual(False)

    # Reads a single byte from magnetometer registers
    # Magnetometer must be in manual mode
    def readFromMagRegister(self,register):
        return self.readBurstFromMagRegister(register,1)[0]

    # Reads count bytes from magnetometer registers
    # Magnetometer must be in manual mode
    # Allowed values for count are 1,2,6,8
    def readBurstFromMagRegister(self,register,count):
        self.setMagManual(True)
        if count==8:
            rd_burst=0x03
        elif count==6:
            ...
            #Invalid, read only one byte
            count = 1
            rd_burst = 0x00
        self._reg_write_bits(registers.MAG_IF_1,rd_burst,definitions.MAG_IF_0_MAG_RD_BURST_BIT,definitions.MAG_IF_0_MAG_RD_BURST_LEN)
        #Write the register address to MAG_IF[2], which will trigger the actual read operation
        self._reg_write(registers.MAG_IF_2, register)
        #Wait for write to complete by observing bit in status register
        while (self._reg_read_bits(registers.STATUS, definitions.STATUS_MAG_MAN_OP, 1)):
            sleep_ms(1)
        #Data is now written starting at register MAG_X_L. Read out all the registers and return
        data = []
        read_reg=registers.MAG_X_L
        while count>0:
            data.append(self._reg_read(read_reg))
            read_reg = read_reg + 1
            count = count -1
        self.setMagManual(False)
        return data

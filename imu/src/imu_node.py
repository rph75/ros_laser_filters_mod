...
from i2c.srv import i2c_read_byte, i2c_read_byteResponse,i2c_write_byte, i2c_write_byteResponse,i2c_read_block, i2c_read_blockResponse
...
class Driver:
    ...
    def __init__(self, addr):
        self.i2c_read_byte_proxy = rospy.ServiceProxy('/i2c_service/read_byte', i2c_read_byte)
        self.i2c_write_byte_proxy = rospy.ServiceProxy('/i2c_service/write_byte', i2c_write_byte)
        self.i2c_read_block_proxy = rospy.ServiceProxy('/i2c_service/read_block', i2c_read_block)
        self.addr = addr
        ...
        while (1 != self._reg_read_bits(registers.PMU_STATUS, definitions.ACC_PMU_STATUS_BIT,
        ...
        sleep_ms(1)
        # Wait for power-up to complete
        while (1 != self._reg_read_bits(registers.PMU_STATUS, definitions.GYR_PMU_STATUS_BIT,
                                        definitions.GYR_PMU_STATUS_LEN)):
            sleep_ms(200)
            pass
        sleep_ms(1)

        self.setFullScaleGyroRange(definitions.GYRO_RANGE_250)
    ...
    def set_gyro_dlpf_mod(self, emode):
        return self._reg_write_bits(registers.GYRO_CONF, mode, definitions.GYRO_DLPF_SEL_BIT,
    ...
    def setAccelDLPFMode(self, mode):
        return self._reg_write_bits(registers.ACCEL_CONF, mode, definitions.ACCEL_DLPF_SEL_BIT,
    ...
    def getFullScaleAccelRange(self):
        return self._reg_read_bits(registers.ACCEL_RANGE, definitions.ACCEL_RANGE_SEL_BIT,
    ...
    def getMotionDetectionDuration(self):
        return 1 + self._reg_read_bits(registers.INT_MOTION_0, definitions.ANYMOTION_DUR_BIT,
    ...
    def getIntMotionEnabled(self):
        return 0 != (
            self._reg_read_bits(registers.INT_EN_0, definitions.ANYMOTION_EN_BIT, definitions.ANYMOTION_EN_LEN))
    ...
    # Reads the entire fifo data into an array
    # each element in the array represents one 16 bit value
    # if gyro FIFO enabled and accel FIFO disabled:
    #    gx,gy,gz,...
    # if gyro FIFO disabled and accel FIFO enabled:
    #    ax,ay,az,...
    # if gyro FIFO enabled and accel FIFO enabled:
    #    gx,gy,gz,ax,ay,az,...
    def getFIFOData(self):
        raw = self._regs_read(registers.FIFO_LENGTH_0, 2)
        count = unpack('<h', bytes(raw))[0] & 0x0fff
        data = []
        while count > 0:
            # smbus driver limitation to read only 32 bytes at a time (why the hell!?)
            # and if we did not read a full unit of 6 or 12 bytes, it will be read again in next iteration
            # therefore cap at 24, as this is a multiple of both 6 and 12
            # note: if also a magnetometer would be connected, we'd have to change this
            readnow = min(24, count)
            raw = self._regs_read(registers.FIFO_DATA, readnow)
            unpackstr = "<" + str(readnow >> 1) + "h"
            unpacked = unpack(unpackstr, bytes(raw))
    ...
    def getXPosShockDetected(self):
        status = self._reg_read(registers.INT_STATUS_3)
        return 0 != (not (status & (1 << definitions.HIGH_G_SIGN_BIT)) and (
                    status & (1 << definitions.HIGH_G_1ST_X_BIT)))
    ...
    def getYPosShockDetected(self):
        status = self._reg_read(registers.INT_STATUS_3)
        return 0 != (not (status & (1 << definitions.HIGH_G_SIGN_BIT)) and (
                    status & (1 << definitions.HIGH_G_1ST_Y_BIT)))
    ...
    def getZPosShockDetected(self):
        status = self._reg_read(registers.INT_STATUS_3)
        return 0 != (not (status & (1 << definitions.HIGH_G_SIGN_BIT)) and (
                    status & (1 << definitions.HIGH_G_1ST_Z_BIT)))
    ...
    def getXNegMotionDetected(self):
        ...
        return 0 != ((status & (1 << definitions.ANYMOTION_SIGN_BIT)) and (
                    status & (1 << definitions.ANYMOTION_1ST_X_BIT)))
    ...
    def getXPosMotionDetected(self):
        status = self._reg_read(registers.INT_STATUS_2)
        return 0 != (not (status & (1 << definitions.ANYMOTION_SIGN_BIT)) and (
                    status & (1 << definitions.ANYMOTION_1ST_X_BIT)))
    ...
    def getYNegMotionDetected(self):
        status = self._reg_read(registers.INT_STATUS_2)
        return 0 != ((status & (1 << definitions.ANYMOTION_SIGN_BIT)) and (
                    status & (1 << definitions.ANYMOTION_1ST_Y_BIT)))
    ...
    def getZNegMotionDetected(self):
        status = self._reg_read(registers.INT_STATUS_2)
        return 0 != ((status & (1 << definitions.ANYMOTION_SIGN_BIT)) and (
                    status & (1 << definitions.ANYMOTION_1ST_Z_BIT)))
    ...
    # Get 3-axis gyro readings.
    # These registers store the most recent gyro measurements.
    # Gyro measurements are written to these registers at the Output Data Rate
    # as configured by @see get_gyro_rate()
    #
    # The gyro measurement registers, along with the temperature
    # measurement registers, accelerator measurement registers, and external sensor
    # data registers, are composed of two sets of registers: an internal register
    # set and a user-facing read register set.
    #
    # The data within the gyro sensors' internal register set is always
    ...
    #
    # Each 16-bit gyro measurement has a full scale configured by
    # @setFullScaleGyroRange. For each full scale setting, the gyro'
    # sensitivity per LSB is shown in the table below:
    #
    # <pre>
    # Full Scale Range | LSB Sensitivity
    # -----------------+----------------
    # +/- 2000 degrees/sec       | 16.4 LSB/(deg/sec)
    # +/- 1000 degrees/sec       | 32.8 LSB/(deg/sec)
    # +/-  500 degrees/sec       | 65.6 LSB/(deg/sec)
    # +/-  250 degrees/sec      | 131.2 LSB/(deg/sec)
    # +/-  125 degrees/sec      | 262.4 LSB/(deg/sec)
    # </pre>
    # @param x 16-bit signed integer container for X-axis angular velocity
    # @param y 16-bit signed integer container for Y-axis angular velocity
    # @param z 16-bit signed integer container for Z-axis angular velocity
    # @see registers.GYRO_X_L
    def getGyro(self):
    ...
    def _reg_write(self, reg, data):
        #TODO: Error handling
        i2c_response = self.i2c_write_byte_proxy(address=self.addr, register=reg, byte=data)

    def _reg_read(self, reg):
        i2c_response = self.i2c_read_byte_proxy(address=self.addr, register=reg)
        return i2c_response.byte

    def _regs_read(self, reg, n,block_size=32):
        i2c_response = self.i2c_read_block_proxy(address=self.addr, register=reg, count=n, block_size=block_size)
        return i2c_response.data
...
def callback_tim(timer_event):
    ...
    # For now, we simply take the last entry

    # In the future, if we ever integrate the data in the consumer,
    # we could read the entire fifo using
    # data=sensor.getFIFOData()
    motion = sensor.getMotion6()
    imu = Imu()
    imu.header.stamp=rospy.Time.now()
    imu.header.frame_id="imu_frame"
    imu.angular_velocity.x=deg_to_rad(motion[0]/GYRO_SCALE_RANGE_125)
    imu.angular_velocity.y=deg_to_rad(motion[1]/GYRO_SCALE_RANGE_125)
    imu.angular_velocity.z=deg_to_rad(motion[2]/GYRO_SCALE_RANGE_125)
    imu.linear_acceleration.x=motion[3]/ACCEL_SCALE_RANGE_2*MSECSQR_PER_G
    imu.linear_acceleration.y=motion[4]/ACCEL_SCALE_RANGE_2*MSECSQR_PER_G
    imu.linear_acceleration.z=motion[5]/ACCEL_SCALE_RANGE_2*MSECSQR_PER_G
    pub.publish(imu)

def init():
    global sensor
    sensor = Driver(0x69)
    sensor.set_gyro_rate(definitions.GYRO_RATE_25HZ)
    sensor.set_accel_rate(definitions.ACCEL_RATE_25HZ)
    sensor.setFullScaleGyroRange(definitions.GYRO_RANGE_125)
    sensor.setFullScaleAccelRange(definitions.ACCEL_RANGE_2G)

    sensor.setFIFOHeaderModeEnabled(False)
    sensor.setGyroFIFOEnabled(True)
    # For now, we don't need acceleration info
    sensor.setAccelFIFOEnabled(False)
...
if __name__ == '__main__':
    ...
    rospy.wait_for_service('/i2c_service/read_byte')
    rospy.wait_for_service('/i2c_service/read_block')
    rospy.wait_for_service('/i2c_service/write_byte')
    ...
    rospy.loginfo(rospy.get_name() + " imu node started.")
    rospy.spin()


#To access magnetometer:
#Connect ASC(bmi) to SCL(bmm)
#Connect ASD(bmi) to SDA(bmm)
#Set the device address in MAG_IF(bmi) to 0x13, because both SDO(bmm) and CSB(bmm) are pulled up
#Set the mag_pmu_status reg 0x03(bmi) to normal (default is suspended)
#Init the magnetometer via MAG_IF(bmi#Set mag_odr to configure collection frequency
#Set mag_manual_en to disabled in MAG_IF[1](bmi) -> this will enter the data mode

# In magnetometer itself
# Set power control bit to 1 to disabled suspend (0x4b)
#Set ODR (0x4c)
#Set operation mode (0x4c)
#Set repetition for x/y (0x51)
#Set repetition for z (0x52)
#Set mag_manual_en to false (0x4b)
